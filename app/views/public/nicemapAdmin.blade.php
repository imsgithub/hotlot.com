...
<html lang="en">
    ...
    <div class="overlay">
        ...
        <div class="map-wrapper">
            ...
            <div id="admin-log">
              <span>Лог админа новые расчеты</span>
              <div></div>
            </div>
            <div id="admin-log2">
              <span>Лог админа старые расчеты</span>
              <div></div>
            </div>
            @endif
        </div>
            ...
            var data = {
                ...
                render: function(){
                    ...
                    if (temp==='UAUA') {
                        ...
                        if (uaCoeffs[uacoeff]===undefined) {
                            if (uaCoeffs[reverse_uacoeff]===undefined) {
                                ...
                                data.shitCoeffs.uacoeff = 1;
                            } else {
                                data.coeffs.uacoeff = uaCoeffs[reverse_uacoeff];
                                data.shitCoeffs.uacoeff = uaCoeffs[reverse_uacoeff];
                            }
                        } else {
                            data.coeffs.uacoeff = uaCoeffs[uacoeff];
                            ata.shitCoeffs.uacoeff = uaCoeffs[uacoeff];
                        }
                    } else {
                      data.coeffs.uacoeff = 1;
                      data.shitCoeffs.uacoeff = 1;
                    ...
                    if (rules.countries[temp]!==undefined) {
                        ...
                        this.shitCoeffs.country = rules.countries[temp];
                    } else if (rules.countries[reverse_temp]!==undefined) {
                        this.coeffs.country = rules.countries[reverse_temp];
                        this.shitCoeffs.country = rules.countries[reverse_temp];
                    };
                    ...
                    if (h <= 3.3 && h > 2.8) { //if ( h<=3.3 && h>2.8)
                      this.shitCoeffs.load = 1.2; //log('Коэффициент - 1,2');
                    } else {
                      if (m <= 22 && m >= 10) { //if ( m<=22 && m>=10)
                        this.shitCoeffs.load = 1;
                      } else if ( m < 10 && m >= 7 ) {
                        if (l <= rules.l.g && l > rules.l.f) { //if ( l<=13.6 && l>10 )
                          this.shitCoeffs.load = rules.load.f; //log('Коэффициент - 1')
                        } else if (l <= rules.l.f && l > rules.l.e) { //if ( l<=10 && l>7 )
                          this.shitCoeffs.load = rules.load.e; //log('Коэффициент - 0.8')
                        }
                      } else if ( m < 7 ) {
                        if (l <= rules.l.e && l > rules.l.d) {//if ( l<=7 && l>5 )
                          this.shitCoeffs.load = .6; //log('Коэффициент - 0.6')
                        } else if (l <= rules.l.d && l > rules.l.e) { // if ( l<=5 && l>3 )
                          this.shitCoeffs.load = rules.load.c; //log('Коэффициент - 0.5')
                        } else if (l <= rules.l.e && l > rules.l.b) { //if ( l<=3 && l>2 )
                          this.shitCoeffs.load = .4; //log('Коэффициент - 0.4')
                        } else if (l <= rules.l.b ) { //if ( l<=2 )
                          this.shitCoeffs.load = rules.load.b; //log('Коэффициент - 0.2')
                        }
                      }
                    }
                    ...
                    this.shitPrice = this.shitCoeffs.load*this.shitCoeffs.distance*this.shitCoeffs.country*this.output.dist*this.currency.value*this.cargo.type.value*this.shitCoeffs.uacoeff;
                },
                showAll: function() {
                    this.render();
...
//                            'Цена: ' + this.price*0.001 + '\n';
                    ...
                    this.output.shitContent = this.output.content;
                    ...
                    this.output.shitContent += 'Длина маршрута, км: ' + this.output.dist*0.001 + '<br>' +
                            'Страны: ' + this.output.countries.start + '-' + this.output.countries.end + '<br>' +
                            'Коэффициент расстояния: ' + this.shitCoeffs.distance + '<br>' +
                            'Коэффициент страны: ' + this.shitCoeffs.country + '<br>' +
                            'Коэффициент груза: ' + this.shitCoeffs.load + '<br>' +
                            'Параметры груза: длина - '+this.cargo.l.value+', ширина - '+this.cargo.w.value+', высота - '+this.cargo.h.value+', вес - '+this.cargo.m.value + '.<br>';
                    for( var i = 0; i< data.cargo.type.options.length; i++) {
                        if(this.cargo.type.value===this.cargo.type.options[i].value) {
                          this.output.content += 'Тип перевозки: ' + this.cargo.type.options[i].textContent + '<br>';
                          this.output.shitContent += 'Тип перевозки: ' + this.cargo.type.options[i].textContent + '<br>';
                        }
                    };
                    this.output.content += 'Цена: ' + this.price*0.001 + ' ' + this.currency.name;
                    this.output.shitContent += 'Цена: ' + this.shitPrice*0.001 + ' ' + this.currency.name;
                    var outs = {
                    ...
                    if ($('#admin-log').length>0) {
                      ...
                      $('#admin-log2 div').html(this.output.shitContent);
                    }
//                    outs.price.innerHTML = 'от ' + Math.round(this.price*0.001-this.price*0.0001)+ ' до ' +Math.round(this.price*0.001+this.price*0.0001) + ' ' + this.currency.name;
                },
            };
